...
| dbpMoodle.backup.retention_time | string | `"1M"` | Defines the maximum age of a backup before it is deleted |
...
| extraObjects | list | `[]` | List of arbitrary Kubernetes objects to deploy alongside the release. -- Each entry is a full manifest which requires apiVersion, kind and metadata.name. -- namespace defaults to the release namespace unless set in metadata. -- Default labels (app.kubernetes.io/..., helm.sh/chart) are added, user labels take precedence. |
